...
const DefaultLayout = ({children} : LayoutProps) : ReactElement => {
  ...
  useEffect(() => {
    ...
    checkSessionId(sessionId).then(result => {
      if (!result.logged_in) {
        ...
        setRightElements(
          <Link href="/login"><Dropdown overlay={login_menu} placement="bottomRight"><LoginOutlined className="NavbarIcon" /></Dropdown></Link>
        );
      } else {
        setRightElements(
          <Row gutter={24} align='middle'>
            <Col><Badge size="default" count={result.prompts} title={`You have ${result.prompts} unread message${((result.prompts as Number) > 1) ? "s" : ""}.`} ><MailOutlined className="NavbarIcon" /></Badge></Col>
            <Col><Title level={4}>{result.username}</Title></Col>
            <Col><Link href="/logout" title="Logout"><LogoutOutlined className="NavbarIcon" /></Link></Col>
          </Row>
        );
      }
    }).catch(err => {
      setRightElements(
        <WarningOutlined className="NavbarIcon" title="Could not connect to remote API." />
      );
      console.error(err);
    });
  ...
  return (<>
    <Layout>
      <Header>
        <Row gutter={48} align='middle' className="Navbar">
      ...
      <Footer style={{ textAlign: "center" }}>
        <Text>
          Powered by React.js & Antd.<br />[<Link href='https://github.com/c7w/ExamManager' target='_blank'>ExamManager</Link>] © c7w 2021.
        </Text>
      </Footer>
    </Layout>
  </>);
